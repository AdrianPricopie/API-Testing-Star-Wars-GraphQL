...
Displays an image of the test report generated by Newman, indicating the number of tests passed and failed.

![Report](https://github.com/AdrianPricopie/API-Testing-Star-Wars-GraphQL/blob/main/Report%20Newman.png)

# Bug report

1.**When running the test script "Verify error message for id", the following error occurs:**

Steps to Reproduce
- Execute the test script "Verify error message for id".
- Observe the test result.

Expected Behavior
The test script should verify that when providing an invalid ID, the response should include an error message indicating that the film with the provided ID does not exist.

Actual Behavior
The test script encounters a TypeError instead of verifying the expected error message.
![Bug](https://github.com/AdrianPricopie/API-Testing-Star-Wars-GraphQL/blob/main/Screenshot%202024-04-20%20at%2012.43.29.png)
